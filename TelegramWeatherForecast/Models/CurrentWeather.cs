...
using TelegramWeatherForecast.Extensions;

namespace TelegramWeatherForecast.Models
{
    internal class CurrentWeather
    {
        ...
        public override string ToString()
        {
            return 
                ...
                $"🔹 Температура: {(int)Temperature}°C (Ощущается как {(int)FeelsLike}°C)\n" +
                $"🔹 Влажность {Humidity}%25\n" +
                $"🔹 Давление {Pressure} мм рт.с.\n" +
                $"🔹 Ветер {string.Format("{0:0.0}", WindSpeed)} м/c {WindDirection}";
        }
    }
}
